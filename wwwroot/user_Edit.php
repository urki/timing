...
if ($_REQUEST["submit"]!='') {
	...
	if ($sex  and $full_name and $city and $btime and event_id ) {
		...
		$sql = "update `timming`.`users` set `full_name`=$full_name, `birthdate`=$btime,`sex`= '$sex',
                        `city`='$city',`club`='$club',`event_id`='$event',`number`='$number'     where `id_user`=$id_user ";
                                
		$db->query($sql);
		$message = "Popravek/štartna številka uspešno dodana!";
		$success = "true";
