...
$age="19";
$res = get_user_group("M",$age,"<=");
if ($res->affected_rows()>0) {
	$titleAge=explode(" ",$age);
	create_results($res,"  mladina M do ".$titleAge[0]." let");
	$age=0;
}

//Pogledamo rezultate za Moske pod 40 let//
$age="20 and 40";
$res = get_user_group("M",$age,"between");
if ($res->affected_rows()>0) {
	$titleAge=explode(" ",$age);
	create_results($res,"  M do ".$titleAge[2]." let");
	$age=0;
}
...
if ($res->affected_rows()>0) {
	$titleAge=explode(" ",$age);
	create_results($res,"  M veterani do ".$titleAge[2]." let");
	$age=0;
}
...
if ($res->affected_rows()>0) {
	$titleAge=explode(" ",$age);
	create_results($res,"  mladina Ž do ".$titleAge[0]." let");
	$age=0;
}
