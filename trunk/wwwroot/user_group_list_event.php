...
$db->query($sql);
	...
	if ($res->affected_rows()>0) {
                $titleAge=explode(" ",$age);
                create_results($res,$event_name."   - M do ".$titleAge[2]." let");
                $age=0;
               }
	...
	if ($res->affected_rows()>0) {
                $titleAge=explode(" ",$age);
                create_results($res,$event_name."   - M veterani do ".$titleAge[2]." let");
                $age=0;
               }
...
//Pogledamo rezultate za Mladino Ž do 19let//
        $age="19";
	$res = get_user_group_event("F",$age,"<=");
	if ($res->affected_rows()>0) {
                $titleAge=explode(" ",$age);
        ...
        $age="20 and 40";
	$res = get_user_group_event("F",$age,"between");
	if ($res->affected_rows()>0) {
                $titleAge=explode(" ",$age);
                create_results($res,$event_name."  - Ž do ".$titleAge[2]." let");
                $age=0;
               }
	...
	if ($res->affected_rows()>0) {
                $titleAge=explode(" ",$age);
                create_results($res,$event_name." - Ž veterani do ".$titleAge[2]." let");
                $age=0;
               }
